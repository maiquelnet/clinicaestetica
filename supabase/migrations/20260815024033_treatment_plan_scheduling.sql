alter table public.planos_tratamento
  ...
  add column if not exists profissional_id uuid references public.perfis(id) on delete set null;

alter table public.planos_tratamento
  drop constraint if exists planos_tratamento_frequencia_check,
  add constraint planos_tratamento_frequencia_check
    check (frequencia is null or frequencia in ('diario', 'semanal', 'mensal', 'intervalo')),
  drop constraint if exists planos_tratamento_intervalo_check,
  add constraint planos_tratamento_intervalo_check
    check (frequencia <> 'intervalo' or intervalo_dias > 0);

alter table public.itens_plano_tratamento
  add column if not exists inicio_previsto_em timestamptz,
  add column if not exists fim_previsto_em timestamptz,
  add column if not exists ajuste_manual boolean not null default false,
  add column if not exists situacao text not null default 'planejado',
  add column if not exists arquivado_em timestamptz;

alter table public.itens_plano_tratamento
  drop constraint if exists itens_plano_tratamento_situacao_check,
  add constraint itens_plano_tratamento_situacao_check
    check (situacao in ('planejado', 'aguardando_reagendamento', 'cancelado_plano')),
  drop constraint if exists itens_plano_tratamento_periodo_check,
  add constraint itens_plano_tratamento_periodo_check
    check (fim_previsto_em is null or inicio_previsto_em is null or fim_previsto_em > inicio_previsto_em);

create unique index if not exists itens_plano_numero_unique_idx
  on public.itens_plano_tratamento (plano_tratamento_id, numero_sessao)
  where numero_sessao is not null;

create unique index if not exists itens_plano_agendamento_unique_idx
  on public.itens_plano_tratamento (agendamento_id)
  where agendamento_id is not null;

create index if not exists planos_tratamento_clinica_status_idx
  on public.planos_tratamento (clinica_id, status)
  where arquivado_em is null;

create index if not exists itens_plano_clinica_inicio_idx
  on public.itens_plano_tratamento (clinica_id, inicio_previsto_em)
  where arquivado_em is null;

create index if not exists itens_plano_plano_idx
  on public.itens_plano_tratamento (plano_tratamento_id, numero_sessao);
...
revoke execute on function public.salvar_plano_tratamento_com_cronograma(
  uuid, uuid, uuid, uuid, text, integer, numeric, numeric, text, date, time,
  text, integer, boolean, boolean, text, jsonb
) from public, anon;

grant execute on function public.salvar_plano_tratamento_com_cronograma(
  uuid, uuid, uuid, uuid, text, integer, numeric, numeric, text, date, time,
  text, integer, boolean, boolean, text, jsonb
) to authenticated;

create or replace function private.sincronizar_item_plano_por_agendamento()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
declare
  ...
  v_plano_status text;
begin
  if pg_catalog.current_setting('app.tratamento_cronograma', true) = '1' then
    return new;
  end if;

  select i.id, p.status
    into v_item_id, v_plano_status
  from public.itens_plano_tratamento i
  join public.planos_tratamento p on p.id = i.plano_tratamento_id
  where i.agendamento_id = new.id
    and i.arquivado_em is null;
  ...
  if new.status = 'cancelado' and old.status is distinct from new.status then
    update public.itens_plano_tratamento
    set situacao = case when v_plano_status = 'em_andamento' then 'aguardando_reagendamento' else 'cancelado_plano' end,
        ajuste_manual = true,
        atualizado_em = pg_catalog.now()
    where id = v_item_id;
  elsif new.inicio_em is distinct from old.inicio_em
    or new.fim_em is distinct from old.fim_em
    or (old.status = 'cancelado' and new.status <> 'cancelado') then
    update public.itens_plano_tratamento
    set inicio_previsto_em = new.inicio_em,
        fim_previsto_em = new.fim_em,
        situacao = 'planejado',
        ajuste_manual = true,
        atualizado_em = pg_catalog.now()
    where id = v_item_id;
...
drop trigger if exists trg_agendamentos_sincronizar_item_plano on public.agendamentos;
create trigger trg_agendamentos_sincronizar_item_plano
after update of inicio_em, fim_em, status on public.agendamentos
for each row execute function private.sincronizar_item_plano_por_agendamento();
